...
# ⭐ 이 파일이 import 될 때 자동으로 실행되어 테이블을 만듭니다.
init_db()

# ==========================================
# 2. 사용자 관련 함수 (회원가입, 로그인 등)
# ==========================================
...
def check_login(username, password):
    ...
    user = c.fetchone()
    conn.close()
    return user # 성공하면 사용자 정보, 실패하면 None 반환

def is_username_taken(username):
    # 여기서 에러가 났던 것입니다. 이제 init_db()가 먼저 실행되니 에러가 안 날 겁니다.
    conn = get_connection()
    c = conn.cursor()
...
def get_user_info(user_id):
    ...
    if row:
        return {
            ...
            "email": row[4], "address": row[5], "birthdate": row[6], "created_at": row[7]
        }
    return None
...
def delete_user_account(user_id):
    conn = get_connection()
    c = conn.cursor()
    c.execute('DELETE FROM recipes WHERE user_id=?', (user_id,)) # 레시피 먼저 삭제
    c.execute('DELETE FROM users WHERE id=?', (user_id,))      # 회원 삭제
    conn.commit()
    conn.close()
...
def get_user_recipes(user_id):
    conn = get_connection()
    conn.row_factory = sqlite3.Row # 결과를 딕셔너리처럼 쓰게 해줌
    c = conn.cursor()
